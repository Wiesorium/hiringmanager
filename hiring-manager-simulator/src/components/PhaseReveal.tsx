...
export function PhaseReveal() {
    ...
    const realManagerChoice =
        candidates.find(c => c.outcome.type === 'success' && c.id !== finalChoice) ||
        candidates.find(c => c.outcome.type === 'success') ||
        candidates[0];
        ...
        <AnimatePresence mode="wait">
            {!showReward ? (
                ...
                >
                    ...
                    >
                        <button
                            onClick={() => { setShowReward(true); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                            className="flex items-center gap-3 px-10 py-4 bg-ink text-paper rounded-full font-bold text-lg hover:bg-black transition-all hover:scale-105 shadow-lg"
                        >
                            Wie geht's jetzt weiter? <ArrowRight className="w-5 h-5" />
                        </button>
                        <p className="text-xs text-muted">Lernmomente · Blog-Tipps · Gesprächsangebot</p>
                    </motion.div>
                </motion.div>
            ) : (
                /* ── Step 2: Reward / CTA page ── */
                <motion.div
                    key="reward"
